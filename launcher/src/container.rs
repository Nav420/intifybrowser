...
impl Vault {
    ...
    fn outer_region_size(&self) -> Result<u64> {
        ...
        // body/2 is the *ciphertext* size of the outer region; convert to
        // plaintext so the decrypt/commit loops don't overrun into the
        // hidden region (each 64 KiB plaintext chunk adds 16 bytes of GCM tag).
        Ok(plaintext_from_ct(body / 2))
    }

    fn hidden_region_size(&self) -> Result<u64> {
        let total = self.file.metadata()?.len();
        let body = total.saturating_sub(HEADER_RESERVED);
        Ok(plaintext_from_ct(body - body / 2))
    }

    fn region_offset(&self, slot: Slot) -> Result<u64> {
        Ok(match slot {
            Slot::Outer => HEADER_RESERVED,
            // The hidden region starts immediately after the outer region's
            // *ciphertext* on disk, so use body/2 (ct size), not plaintext size.
            Slot::Hidden => {
                let total = self.file.metadata()?.len();
                let body = total.saturating_sub(HEADER_RESERVED);
                HEADER_RESERVED + body / 2
            }
        })
    }
...
pub fn init(
    ...
    // IMPORTANT: the hidden region offset is ciphertext-based (outer_ct_size),
    // not plaintext-based, because each chunk adds TAG_LEN bytes of GCM tag.
    let outer_pt = nominal_size / 2;
    let outer_ct = ct_size_for_plaintext(outer_pt);
    write_random_region(&mut f, HEADER_RESERVED, outer_pt, &master_outer, *b"OUTR", Slot::Outer)?;
    write_random_region(&mut f, HEADER_RESERVED + outer_ct, outer_pt, &master_hidden, *b"HIDN", Slot::Hidden)?;
...
/// Convert an on-disk ciphertext region size to its plaintext equivalent.
/// Each CHUNK bytes of plaintext produces CHUNK + TAG_LEN bytes of ciphertext.
fn plaintext_from_ct(ct: u64) -> u64 {
    let chunk_ct = (CHUNK + TAG_LEN) as u64;
    (ct / chunk_ct) * CHUNK as u64
}

/// Convert a plaintext size to the ciphertext bytes it will occupy on disk.
/// Used to compute the correct seek offset for the hidden region.
fn ct_size_for_plaintext(plaintext: u64) -> u64 {
    // Assumes plaintext is an exact multiple of CHUNK (guaranteed by our
    // init routines which accept sizes like 512 MiB, 1 GiB, etc.)
    let chunks = plaintext / CHUNK as u64;
    chunks * (CHUNK + TAG_LEN) as u64
}
...
/// Like `init()` but takes passwords as `&str` (no terminal prompting) and
/// writes zero plaintext for the vault body so the GUI layer can detect a
/// fresh vault (tar_len prefix == 0) and let the browser create a clean profile.
pub fn init_with_passwords(
    path: &Path,
    nominal_size: u64,
    password: &str,
    ...
    let secret_hidden = match hidden_password {
        ...
        None => {
            let mut r = vec![0u8; 64];
            crypto::random_bytes(&mut r);
            crypto::UserSecret { mixed: zeroize::Zeroizing::new(r) }
        }
    };
    ...
    let enc_master_outer =
        crypto::aead_seal(&uk_outer, &nonce_outer, b"ifb:slot:outer", master_outer.as_ref())?;
    let enc_master_hidden =
        crypto::aead_seal(&uk_hidden, &nonce_hidden, b"ifb:slot:hidden", master_hidden.as_ref())?;
    ...
    // Write zero plaintext (encrypted) so the GUI reads tar_len == 0 on first open.
    // Hidden region offset is ciphertext-based (same fix as in `init`).
    let outer_pt = nominal_size / 2;
    let outer_ct = ct_size_for_plaintext(outer_pt);
    write_zero_region(&mut f, HEADER_RESERVED, outer_pt, &master_outer, *b"OUTR", Slot::Outer)?;
    write_zero_region(&mut f, HEADER_RESERVED + outer_ct, outer_pt, &master_hidden, *b"HIDN", Slot::Hidden)?;

    f.sync_all()?;
    Ok(())
}

/// Encrypt a region of zero plaintext. The ciphertext is indistinguishable
/// from random (AES-GCM with a fresh key/nonce), so vault deniability is
/// preserved while the GUI can detect tar_len == 0 after decryption.
fn write_zero_region(
    f: &mut File,
    offset: u64,
    len: u64,
    master_key: &crypto::SecretKey,
    domain: [u8; 4],
    slot: Slot,
) -> Result<()> {
    f.seek(SeekFrom::Start(offset))?;
