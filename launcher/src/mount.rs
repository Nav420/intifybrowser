...
//! Windows : Temp directory under %TEMP%. A future revision will use the
//!           ImDisk driver for a true RAM disk; the temp-dir backend is
//!           functionally equivalent and simpler to deploy without a
//!           kernel driver.
//!
...
pub struct RamMount {
    path: PathBuf,
    /// On Linux: true means the tmpfs is still mounted and needs umount2.
    /// On Windows: true means the temp dir exists and needs removal.
    mounted: bool,
}
...
impl RamMount {
    ...
    pub fn create(_size_bytes: u64) -> Result<Self> {
        ...
        std::fs::create_dir_all(&path).context("create temp mount dir")?;
        Ok(Self { path, mounted: true })
    }

    #[cfg(not(any(target_os = "linux", windows)))]
    pub fn create(_size_bytes: u64) -> Result<Self> {
        anyhow::bail!("RAM mount backend not implemented for this platform")
    }
    ...
    pub fn unmount(mut self) -> Result<()> {
        if self.mounted {
            std::fs::remove_dir_all(&self.path).context("remove temp mount dir")?;
            self.mounted = false;
        }
        Ok(())
    }
